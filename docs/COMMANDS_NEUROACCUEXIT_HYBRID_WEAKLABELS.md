...
| v0.3 | Per-label threshold tuning on validation for v0.1 and/or v0.2 checkpoints | Calibrate the best trained model |

The generated hybrid manifest is treated as the input dataset. Do not rebuild TATA-LAWYER or alter the frozen v0.10.1 hybrid policy during these runs.

---

## 0. Start from the correct branch

Run from the repository root:

```powershell
git fetch origin
git checkout neuroaccuexit_hybrid_weaklabels
git pull
```

If your local branch is diverged, use your existing local sync procedure before running the experiments.
...
After training finishes, copy the printed run directory into:
...
This run uses the same architecture and dataset, but enables per-label `pos_weight` capped at 5.0.
...
Use this after each run, replacing `$RunV01` / `$RunV02` if needed:
...
Select the better training run mainly by strict validation/test Macro-F1, Samples-F1, Exact Match, and Hamming Loss. Do not select only by one metric.
...
function Show-TuningSummary($TuneDir) {
  ...
  Write-Host "\nTUNING: $TuneDir"
  [PSCustomObject]@{
    metric = "Macro-F1"; fixed = $fixed.macro_f1; tuned = $tuned.macro_f1; change = $tuned.macro_f1 - $fixed.macro_f1
  }
  [PSCustomObject]@{
    metric = "Micro-F1"; fixed = $fixed.micro_f1; tuned = $tuned.micro_f1; change = $tuned.micro_f1 - $fixed.micro_f1
  }
  [PSCustomObject]@{
    metric = "Samples-F1"; fixed = $fixed.samples_f1; tuned = $tuned.samples_f1; change = $tuned.samples_f1 - $fixed.samples_f1
  }
  [PSCustomObject]@{
    metric = "Exact Match"; fixed = $fixed.exact_match; tuned = $tuned.exact_match; change = $tuned.exact_match - $fixed.exact_match
  }
  [PSCustomObject]@{
    metric = "Hamming Loss"; fixed = $fixed.hamming_loss; tuned = $tuned.hamming_loss; change = $tuned.hamming_loss - $fixed.hamming_loss
  }
}
...
## 7. Expected decision rule

| Result pattern | Selected downstream model |
|---|---|
| v0.1 fixed is strongest | Keep plain BCE baseline |
| v0.1 tuned is strongest | Keep plain BCE + per-label thresholds |
| v0.2 fixed is strongest | Keep capped pos_weight model |
| v0.2 tuned is strongest | Keep capped pos_weight + per-label thresholds |
